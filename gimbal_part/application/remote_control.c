...
bool_t key_pressed(uint16_t judged_key, uint16_t exclude_key){
/**	
	*	@param[in]	judged_key: key to be judged if pressed		exclude_key:	key to be excluded when judging	
	*	@retval			if any exclude_key is pressed, return 0; else if judged_key is pressed, return 1; if no key to be excluded, setup param exclude_key to 0;
	*	@sample 		例如键位x和组合键ctrl+x搭载不同功能，则对于键位x：judged_key=x, exclude_key=ctrl; 对于组合键ctrl+x: judged_key=ctrl|x, exclude_key=0
	*/
		if(exclude_key & rc_ctrl.key.v)
				return 0;
